...
<html>
	<head>
		...
		<title>Car4Rent | Select Location</title>
	...
	<body>
		...
		<div style="margin:50px; font-size:14pt; border-bottom:5px solid darkgray;">
			<nav style="margin-bottom:10px;">
				...
				<span class="header" style="float:none;color:#00361d;background-color:yellow">Select A Location</span> >
				<span class="header" style="float:none;color:grey">Select A Vehicle</span> >
		...
		<div style="margin:50px;">
			<form action="SelectVehicle.php" method="post">
				<?php
					$fname = $_POST['given_name_value'];
					$lname = $_POST['family_name_value'];
					$email = $_POST['email_value'];
					$tel = $_POST['phone_number_value'];
					$put = $_POST['pickupdt'];
					$rt = $_POST['returndt'];
					$loc = $_POST['loc'];
					...
					switch ($loc) {
						...
						case 'Dallas':   
							break;
						default: 
							echo '<p>That is not a valid area. <br/>
					...
					try {
						...
						// perform query
						$query = "SELECT locationName, locationStreet, locationCity, locationState, locationZip FROM locations WHERE locationCity like :loc";  
						$stmt = $db->prepare($query);  
						$stmt->bindParam(':loc', $loc);
						$stmt->execute(); 

						// get number of returned rows  
						echo "<h2>Select a location:</h2>"; 
						...
						while($result = $stmt->fetch(PDO::FETCH_OBJ)) {                                                       
							...
							echo "<br /><button type='submit' name='location' value='".$result->locationName."'>Next</button>";
